"""
Options Flow Scanner Backend - AWS Elastic Beanstalk Version
Optimized for AWS with CORS, HTTPS, and proper error handling
"""
...
class OptionsScanner:
    ...
    def get_options_data(self, symbol):
        ...
        try:
            ...
            # Get nearest expiration (or next week)
            ...
            # Detect conditions (simplified)
            ask_side = (calls['ask'] > calls['bid']).sum() > len(calls) * 0.6
            sweep = (calls['volume'] > calls['openInterest']).sum() > 0
            
            # OTM detection (calls above current price)
            otm_calls = calls[calls['strike'] > current_price * 1.02]
            otm = len(otm_calls[otm_calls['volume'] > 0]) > 0
            
            # IV sync (simplified - check if IV is elevated)
            avg_iv = calls['impliedVolatility'].mean()
            iv_sync = avg_iv > 0.3  # Threshold
            
            # Price still - no big price movement
            price_still = True  # Simplified
            
            # Determine consecutive (mock based on volume)
...
if __name__ == '__main__':
    print("Starting Options Scanner Backend on http://localhost:5000")
    print("Available endpoints:")
    print("  POST /api/scan - Start scanning")
    print("  GET /api/results - Get results")
    print("  GET /api/symbol/<symbol> - Get symbol data")
